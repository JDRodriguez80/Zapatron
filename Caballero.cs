...
namespace Zapatron
{
    public class Caballero : Zapatos
    {
        ...
        string color;
        int cantidad;
        public Caballero(string name, string tipo, double precio, int talla, string color, int cantidad) : base(name, tipo, precio, color, cantidad)
        {
            this.name = name;
            this.tipo = tipo;
            this.precio = precio;
        ...
        {
            if (Enumerable.Range(25, 30).Contains(talla))

            {
                var total = precio * cantidad;
                var porcentaje = total * .13;
