...
namespace Zapatron
{
    public class Ninos : Zapatos
    {
        ...
        string color;
        int cantidad;
        public Ninos(string name, string tipo, double precio, int talla, string color, int cantidad) : base(name, tipo, precio, color, cantidad)
        {
            this.name = name;
            this.tipo = tipo;
            this.precio = precio;
        ...
        {
            if (Enumerable.Range(15, 21).Contains(talla))

            {
                var total = precio * cantidad;
                var porcentaje = total * .08;
